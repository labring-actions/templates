...
| `entries`       | 可选。声明部署完成后平台默认打开的地址（`open`）和默认用于分享的地址（`share`）。见 [解释：`Entries`](#解释entries) |
...
`entries` 是可选项。它告诉平台 **打开** 按钮指向哪个地址，以及用户 **分享** 这个应用时该给出哪个地址。两个值都是完整 URL，支持和其他字段一样的 `${{ }}` 变量。
...
| 键      | 含义                                                         | 省略时                                                                       |
| :------ | :----------------------------------------------------------- | :--------------------------------------------------------------------------- |
| `open`  | 部署者部署完成后第一个进入的页面，例如管理后台或登录页。       | 平台使用模板中 `kind: App` 对象的 `url`；没有的话使用第一个根路径的公网 HTTP 地址。 |
| `share` | 面向最终用户的页面，例如游戏客户端、公开站点，或带参数的加入链接。 | 平台分享和打开相同的地址。                                                   |

只在两个地址不同、或者要分享的地址不是应用根路径时声明 `entries`。不要在 `share` 中放入密钥（token、邀请码），这个地址会离开平台。
