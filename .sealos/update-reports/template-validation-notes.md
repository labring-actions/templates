...
- Rocket.Chat microservices live runtime pass: updated the main Rocket.Chat, account, authorization, DDP streamer, presence, stream hub, and NATS config reloader images after template contract normalization; fixed MongoDB connection secret-key drift by using the MongoDB Service DNS, added authSource=rocketchat, made mongo-init idempotent, and raised the main Rocket.Chat container to the 2Gi resource tier while keeping MongoDB at the docker-to-sealos 512Mi component tier. Template API dry-run/deploy succeeded on ns-dn9ue3wz, MongoDB, NATS, the main app, account, authorization, DDP streamer, presence, and stream hub reached ready, root /, /health, /api/info, /sockjs/info, and /websocket returned HTTP 200, Rocket.Chat 8.5.1 reported SERVER RUNNING, MongoDB contained 83 collections with seeded users and rooms, NATS varz and config reloader were healthy, metrics endpoints returned HTTP 200, and test footprints were cleaned up.
...
- `penpot`: single-template docker-to-sealos consistency check passed after Penpot frontend/backend/exporter image updates, PostgreSQL 16.4 metadata and idempotent pg-init, Redis/PostgreSQL URL composition, object-storage env indirection, Service/Ingress/App schema, resource normalization, and omitting volumeClaimTemplates metadata labels. Live Sealos validation passed for Template API dry-run/deploy, PostgreSQL and Redis Running, backend/exporter/frontend readiness, root and /readyz HTTP 200, anonymous profile API HTTP 200, backend migrations through 0151, Redis authenticated PONG, object storage bucket secret wiring, and cleanup.
...
- `rocketchat-micro`: single-template docker-to-sealos consistency check passed after updating Rocket.Chat microservices and NATS config reloader images while preserving MongoDB database visibility labels, NATS ConfigMap labels and mounts, NATS PID storage, resource ladder values, mongo-init image policy, Service labels, and localized metadata. Live Sealos validation passed for Template API dry-run/deploy, MongoDB, NATS, main app, account, authorization, DDP streamer, presence, and stream hub readiness, public HTTP routes, SockJS/WebSocket routing, MongoDB state, NATS varz/reloader health, metrics endpoints, and cleanup.
...
- `posthog`: single-template docker-to-sealos consistency check passed after updating Redpanda and Zookeeper images while preserving PostgreSQL and Redis database visibility labels, Kafka/Zookeeper/PostHog/Plugins/ClickHouse Service labels, ClickHouse and Plugins ConfigMap file mounts, workload ConfigMap startup scripts, normalized resource ladder values, and the simplified ClickHouse Service topology. Live Sealos validation passed for Template API dry-run/deploy, PostgreSQL and Redis Running, pg-init completion, Redpanda/Zookeeper/ClickHouse readiness, PostHog workload readiness, root redirect, /login and /signup HTTP 200, /_health ok, Redpanda cluster health, ClickHouse posthog database state, Redis PONG, PostgreSQL migration state, capture Kafka connectivity, and cleanup.
...
- `rybbit`: single-template docker-to-sealos consistency check passed after updating ClickHouse to 26.5.3, preserving normalized ConfigMap keys, single-volume file mounts, init container resources, and non-database PostgreSQL wait helper. Live Sealos validation passed for Template API dry-run/deploy, client ConfigMap startup patching of prebuilt Next.js standalone public env values, PostgreSQL Running, pg-init completion, ClickHouse/backend/client readiness with zero restarts, root/login/signup/API HTTP checks, sign-up/session/organization APIs, PostgreSQL public table state, ClickHouse analytics tables, recent log scan, and cleanup.
