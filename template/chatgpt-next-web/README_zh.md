...
访问控制由 `CODE` 输入管理。配置 `CODE` 后，用户在首屏输入逗号分隔列表中的任意访问码。`CODE` 为空时，应用会直接打开。
...
| `CODE` | Web UI 访问码，多个访问码使用英文逗号分隔。 | `否` | `` |
...
| `HIDE_BALANCE_QUERY` | 设置为 `1` 后隐藏余额查询功能。 | `否` | `` |
...
请将私有 API Key 和访问码保存在 Sealos 管理的输入项中。
...
将 `HIDE_BALANCE_QUERY` 设置为 `1`，然后重启 Deployment。
