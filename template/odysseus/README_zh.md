...
Odysseus 是一个自托管 AI 工作空间，支持聊天、自主 Agent、工具、模型服务、邮件、研究、笔记和记忆。此模板会在 Sealos Cloud 上部署 Odysseus、Chroma 向量记忆、SearXNG 搜索、持久化存储和公开 HTTPS 访问入口。

![Odysseus 截图](https://raw.githubusercontent.com/labring-actions/templates/kb-0.9/template/odysseus/website-screenshot.webp)

## 关于托管 Odysseus

Odysseus 作为带认证的 Web 应用运行，使用本地 SQLite 数据、文件存储、Chroma 和 SearXNG。主 Odysseus 服务在 7000 端口提供 Web UI 和 API，Chroma 与 SearXNG 留在 Sealos 集群内网中。

此模板会创建三个 StatefulSet：Odysseus 应用、用于向量记忆的 Chroma、用于元搜索的 SearXNG。持久卷会保存 Odysseus 数据、日志、SSH 状态、Hugging Face 缓存、本地模型服务文件、Chroma 数据和 SearXNG 缓存。
...
Sealos 模板包含 Odysseus 应用容器、Chroma 向量数据库、SearXNG 搜索服务、持久化存储和 HTTPS Ingress。
...
- **持久化存储**：保存应用数据、日志、SSH 状态、模型与缓存目录、Chroma 数据和 SearXNG 缓存。
...
- **一键部署**：通过一个 App Store 模板部署 Odysseus、Chroma、SearXNG、存储、Ingress 和仪表盘入口。
...
1. 打开当前部署的 Canvas。
2. 点击 Odysseus、Chroma 或 SearXNG StatefulSet 资源卡片。
3. 根据工作负载调整 CPU、内存、副本数或存储。
4. 在对话中应用变更。
