...
模板在首次启动时初始化配置并准备分词器缓存，后续重启会保留通过 WebUI 修改的设置。浏览器访问使用自动生成的私有密码，文件工具默认限制在工作区内运行。
...
3. 等待部署完成，通常需要 2-3 分钟。部署完成后，Canvas 会显示网关及其持久化存储。
4. 打开网关资源卡片，复制环境变量 `NANOBOT_WEB_TOKEN` 的值，并妥善保存这个自动生成的密码。
5. 打开应用公共地址，在 **Password** 中输入该值，然后点击 **Connect**。通过共享密码验证后即可进入 WebUI。
...
浏览器会在本地记住访问密码。更换浏览器或清除浏览器存储后，再次输入 `NANOBOT_WEB_TOKEN` 即可连接。网关采用共享访问密码，获得密码的可信用户可以操作同一个智能体工作区。
