...
code-server 可以在浏览器中运行 VS Code，并提供持久化工作区卷。此模板会在 Sealos Cloud 上部署带密码认证、持久化 home 存储和 HTTPS 访问入口的 code-server。
...
code-server 在 `8080` 端口提供 VS Code Web UI。Sealos 模板会创建 StatefulSet、持久化 `/home/coder` 卷、Service、Ingress 和 App 入口。
...
2. 将 `PASSWORD` 设置为强密码。
3. 等待 StatefulSet 就绪，然后从 Sealos Canvas 打开生成的 HTTPS 地址。
4. 在登录页输入 `PASSWORD`。
5. 使用 `/home/coder/workspace` 作为默认持久化工作区。

## 配置说明

| 名称 | 说明 | 必填 | 默认值 |
|------|------|------|--------|
| `PASSWORD` | code-server 登录页使用的密码。 | `是` | `<已隐藏>` |

请将密码保存在 Sealos 管理的输入项中，需要变更访问权限时可在 Canvas 中轮换。

## 扩展

模板按小型交互式 IDE 会话调优。运行语言服务器、安装依赖或执行较重终端任务时，可在 Sealos Canvas 中提高 CPU 和内存。

## 故障排查

### 密码校验失败

确认 Deployment 输入中的当前 `PASSWORD`，需要时从 Canvas 更新该值并重启 StatefulSet。

### 终端或语言服务器较慢

提高 code-server StatefulSet 的 CPU 和内存。大型仓库和语言服务器需要的内存高于基础编辑器。

### 工作区文件丢失

确认文件保存于 `/home/coder/workspace`。该路径由持久化 `/home/coder` 卷保存。
