...
此 Sealos 模板会将 **Lobe Chat Database Version** 部署为 `lobe-chat-db` 应用。部署、网络和存储配置都由仓库中的 Sealos 模板维护。
...
| 名称 | 说明 | 必填 | 默认值 |
|------|------|------|--------|
| `AUTH_LOGTO_ID` | `AUTH_LOGTO_ID` 部署参数。 | `是` | `` |
| `AUTH_LOGTO_ISSUER` | `AUTH_LOGTO_ISSUER` 部署参数。 | `是` | `` |
| `AUTH_LOGTO_SECRET` | `AUTH_LOGTO_SECRET` 部署参数。 | `是` | `<已隐藏>` |
